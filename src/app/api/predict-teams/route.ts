...
function parseMatchesIntoStats(matches: Array<{
  homeTeam: { id: number };
  awayTeam: { id: number };
...
function getForm(matches: Array<{
  homeTeam: { id: number };
  awayTeam: { id: number };
  score: { fullTime: { home: number | null; away: number | null } };
  status: string;
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Build H2H from combined matches
    ...
    const homeStats = parseMatchesIntoStats(homeMatches, homeId);
    const awayStats = parseMatchesIntoStats(awayMatches, awayId);
    const homeForm = getForm(homeMatches, homeId);
    const awayForm = getForm(awayMatches, awayId);
